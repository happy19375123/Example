...
+ (instancetype)standardUserDefaults {
    ...
    dispatch_once(&pred, ^{
        ...
        [sharedInstance loadSelectorSetupDefault];
    });
...
-(void)loadSelectorSetupDefault{
    Class currentClass = [GVUserDefaults class];
    GVUserDefaults *userDefault = [[GVUserDefaults alloc] init];
    ...
    if (currentClass) {
        ...
        for (NSInteger i = 0; i < methodCount; i++) {
            ...
            if ([methodName isEqualToString:@"setupDefaults"]) {
                imp = method_getImplementation(method);
                sel = method_getName(method);
                if (imp != NULL) {
                    fn f = (fn)imp;
                    NSDictionary *dic = f(userDefault,sel);
                    for(NSString *key in dic){
                        [mDic setObject:[dic objectForKey:key] forKey:key];
                    }
                }
            }
        }
    ...
    NSMutableDictionary *mutableDefaults = [NSMutableDictionary dictionaryWithCapacity:[mDic count]];
