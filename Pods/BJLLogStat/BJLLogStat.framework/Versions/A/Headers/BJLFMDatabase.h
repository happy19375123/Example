...
#import "BJLFMResultSet.h"
#import "BJLFMDatabasePool.h"

NS_ASSUME_NONNULL_BEGIN

#if ! __has_feature(objc_arc)
    #define BJLFMDBAutorelease(__v) ([__v autorelease]);
    #define BJLFMDBReturnAutoreleased BJLFMDBAutorelease

    #define BJLFMDBRetain(__v) ([__v retain]);
    #define BJLFMDBReturnRetained BJLFMDBRetain

    #define BJLFMDBRelease(__v) ([__v release]);

    #define BJLFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define BJLFMDBAutorelease(__v)
    #define BJLFMDBReturnAutoreleased(__v) (__v)

    #define BJLFMDBRetain(__v)
    #define BJLFMDBReturnRetained(__v) (__v)

    #define BJLFMDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define BJLFMDBDispatchQueueRelease(__v)
    #else
        #define BJLFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^BJLFMDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);
...
@interface BJLFMDatabase : NSObject
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) BJLFMDBExecuteStatementsCallbackBlock _Nullable)block;
...
- (BJLFMResultSet * _Nullable)executeQuery:(NSString*)sql, ...;
...
- (BJLFMResultSet * _Nullable)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (BJLFMResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (BJLFMResultSet * _Nullable)executeQuery:(NSString *)sql values:(NSArray * _Nullable)values error:(NSError * _Nullable __autoreleasing *)error;
...
- (BJLFMResultSet * _Nullable)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary * _Nullable)arguments;


// Documentation forthcoming.
- (BJLFMResultSet * _Nullable)executeQuery:(NSString *)sql withVAList:(va_list)args;
...
typedef NS_ENUM(int, BJLSqliteValueType) {
    BJLSqliteValueTypeInteger = 1,
    BJLSqliteValueTypeFloat   = 2,
    BJLSqliteValueTypeText    = 3,
    BJLSqliteValueTypeBlob    = 4,
    BJLSqliteValueTypeNull    = 5
};

- (BJLSqliteValueType)valueType:(void *)argv;
...
@interface BJLFMStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
