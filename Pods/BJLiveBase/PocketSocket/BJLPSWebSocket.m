...
- (void)open {
    [self executeWork:^{
        if(_opened || _readyState != BJLPSWebSocketReadyStateConnecting) {
...
- (void)closeWithCode:(NSInteger)code reason:(NSString *)reason {
    [self executeWork:^{
        ...
        BOOL connecting = (_readyState == BJLPSWebSocketReadyStateConnecting);
        _readyState = BJLPSWebSocketReadyStateClosing;
...
- (void)failWithError:(NSError *)error {
    if(error.code == BJLPSWebSocketStatusCodeProtocolError && [error.domain isEqualToString:BJLPSWebSocketErrorDomain]) {
        ...
        [self executeWork:^{
            if(_readyState != BJLPSWebSocketReadyStateClosed) {
                _failed = YES;
