...
@interface BJLPSWebSocketDeflater() {
...
@end
@implementation BJLPSWebSocketDeflater
...
- (BOOL)ensureReady:(NSError *__autoreleasing *)outError {
    if(!_ready) {
        if(deflateInit2(&_stream, Z_DEFAULT_COMPRESSION, Z_DEFLATED, _windowBits, _memoryLevel, Z_FIXED) != Z_OK) {
            BJLPSWebSocketSetOutError(outError, BJLPSWebSocketStatusCodeProtocolError, @"Failed to initialize deflate stream");
            return NO;
        }
        _ready = YES;
    }
