...
typedef NS_ENUM(NSInteger, BJLPSWebSocketReadyState) {
    BJLPSWebSocketReadyStateConnecting = 0,
    BJLPSWebSocketReadyStateOpen,
    BJLPSWebSocketReadyStateClosing,
    BJLPSWebSocketReadyStateClosed
};

@class BJLPSWebSocket;

/**
 *  PSWebSocketDelegate
 */
@protocol BJLPSWebSocketDelegate <NSObject>

@required
- (void)webSocketDidOpen:(BJLPSWebSocket *)webSocket;
- (void)webSocket:(BJLPSWebSocket *)webSocket didFailWithError:(NSError *)error;
- (void)webSocket:(BJLPSWebSocket *)webSocket didReceiveMessage:(id)message;
- (void)webSocket:(BJLPSWebSocket *)webSocket didCloseWithCode:(NSInteger)code reason:(NSString *)reason wasClean:(BOOL)wasClean;
@optional
- (void)webSocketDidFlushInput:(BJLPSWebSocket *)webSocket;
- (void)webSocketDidFlushOutput:(BJLPSWebSocket *)webSocket;
- (BOOL)webSocket:(BJLPSWebSocket *)webSocket evaluateServerTrust:(SecTrustRef)trust;
@end

/**
 *  PSWebSocket
 */
@interface BJLPSWebSocket : NSObject
...
@property (nonatomic, assign, readonly) BJLPSWebSocketReadyState readyState;
@property (nonatomic, weak) id <BJLPSWebSocketDelegate> delegate;
