...
@interface BJLPSWebSocketInflater() {
...
@end
@implementation BJLPSWebSocketInflater
...
- (BOOL)appendBytes:(const void *)bytes length:(NSUInteger)length error:(NSError *__autoreleasing *)outError {
    ...
    do {
        ...
        if(ret == Z_NEED_DICT || ret == Z_DATA_ERROR || ret == Z_MEM_ERROR) {
            BJLPSWebSocketSetOutError(outError, BJLPSWebSocketStatusCodeProtocolError, @"Failed to inflate bytes");
            return NO;
        }
...
- (BOOL)ensureReady:(NSError *__autoreleasing *)outError {
    if(!_ready) {
        if(inflateInit2(&_stream, -MAX_WBITS) != Z_OK) {
            BJLPSWebSocketSetOutError(outError, BJLPSWebSocketStatusCodeProtocolError, @"Failed to initialize inflate stream");
            return NO;
        }
        _ready = YES;
    }
