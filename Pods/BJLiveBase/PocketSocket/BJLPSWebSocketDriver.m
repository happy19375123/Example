...
#import "BJLPSWebSocketInflater.h"
#import "BJLPSWebSocketDeflater.h"
#import "BJLPSWebSocketUTF8Decoder.h"
...
@interface BJLPSWebSocketFrame : NSObject {
@public
    BOOL fin;
    BOOL rsv1;
    BOOL rsv2;
    BOOL rsv3;
    BJLPSWebSocketOpCode opcode;
    BOOL masked;
...
@end
@implementation BJLPSWebSocketFrame

@end

typedef NS_ENUM(NSInteger, BJLPSWebSocketDriverState) {
    BJLPSWebSocketDriverStateHandshakeRequest = 0,
    BJLPSWebSocketDriverStateHandshakeResponse,
    BJLPSWebSocketDriverStateFrameHeader,
    BJLPSWebSocketDriverStateFrameHeaderExtra,
    BJLPSWebSocketDriverStateFramePayload
};

@interface BJLPSWebSocketDriver() {
    NSURLRequest *_request;
    BJLPSWebSocketDriverState _state;
    ...
    BJLPSWebSocketInflater *_inflater;
    BJLPSWebSocketDeflater *_deflater;
    
    uint32_t _utf8DecoderState;
    uint32_t _utf8DecoderCodePoint;
}
@end
@implementation BJLPSWebSocketDriver
...
+ (BOOL)isWebSocketRequest:(NSURLRequest *)request {
    ...
    NSOrderedSet *version = BJLPSHTTPHeaderFieldValues([headers[@"Sec-WebSocket-Version"] lowercaseString]);
    NSOrderedSet *upgrade = BJLPSHTTPHeaderFieldValues([headers[@"Upgrade"] lowercaseString]);
    NSOrderedSet *connection = BJLPSHTTPHeaderFieldValues([headers[@"Connection"] lowercaseString]);
...
+ (instancetype)clientDriverWithRequest:(NSURLRequest *)request {
    return [[self alloc] initWithMode:BJLPSWebSocketModeClient request:request];
}
+ (instancetype)serverDriverWithRequest:(NSURLRequest *)request {
    return [[self alloc] initWithMode:BJLPSWebSocketModeServer request:request];
}
- (instancetype)initWithMode:(BJLPSWebSocketMode)mode request:(NSURLRequest *)request {
    NSParameterAssert(request);
    if((self = [super init])) {
        _mode = mode;
        _state = (_mode == BJLPSWebSocketModeClient) ? BJLPSWebSocketDriverStateHandshakeRequest : BJLPSWebSocketDriverStateHandshakeResponse;
...
- (void)sendText:(NSString *)text {
    ...
    [self writeMessageWithOpCode:BJLPSWebSocketOpCodeText data:data];
}
- (void)sendBinary:(NSData *)binary {
    [self writeMessageWithOpCode:BJLPSWebSocketOpCodeBinary data:binary];
}
- (void)sendCloseCode:(NSInteger)code reason:(NSString *)reason {
    ...
    [self writeMessageWithOpCode:BJLPSWebSocketOpCodeClose data:data];
}
- (void)sendPing:(NSData *)data {
    [self writeMessageWithOpCode:BJLPSWebSocketOpCodePing data:data];
}
- (void)sendPong:(NSData *)data {
    [self writeMessageWithOpCode:BJLPSWebSocketOpCodePong data:data];
}

#pragma mark - Writing

- (void)writeHandshakeRequest {
    NSAssert(_mode == BJLPSWebSocketModeClient, @"Cannot send handshake requests in non client mode");
    NSAssert(_state == BJLPSWebSocketDriverStateHandshakeRequest, @"Cannot start a driver more than once");
...
#pragma clang diagnostic pop
    ...
    // transition state
    _state = BJLPSWebSocketDriverStateHandshakeResponse;
}
- (void)writeHandshakeResponse {
    NSAssert(_state == BJLPSWebSocketDriverStateHandshakeResponse, @"Cannot start a driver more than once");
    ...
    // validate extensions
    NSOrderedSet *extensionComponents = BJLPSHTTPHeaderFieldValues([headers[@"Sec-WebSocket-Extensions"] lowercaseString]);
    if(![self pmdConfigureWithExtensionsHeaderComponents:extensionComponents]) {
        [self failWithErrorCode:BJLPSWebSocketErrorCodeHandshakeFailed reason:@"invalid permessage-deflate extension parameters"];
        return;
    }
    ...
    // transition state
    _state = BJLPSWebSocketDriverStateFrameHeader;
    
    // open
    [_delegate driverDidOpen:self];
}
- (void)writeMessageWithOpCode:(BJLPSWebSocketOpCode)opcode data:(NSData *)data {
    // create header
    NSMutableData *header = [NSMutableData dataWithLength:2];
    uint8_t *headerBytes = header.mutableBytes;
    
    headerBytes[0] |= BJLPSWebSocketFinMask;
    //  headerBytes[0] |= (PSWebSocketRsv2Mask);
    //  headerBytes[0] |= (PSWebSocketRsv3Mask);
    headerBytes[0] |= (BJLPSWebSocketOpCodeMask & opcode);
    
    // determine payload payload
    id payload = data;
    
    // deflate payload
    if(_pmdEnabled && !BJLPSWebSocketOpCodeIsControl(opcode) && [payload length] > 0) {
        // reset deflater if needed
        if((_pmdClientNoContextTakeover && _mode == BJLPSWebSocketModeClient) ||
           (_pmdServerNoContextTakeover && _mode == BJLPSWebSocketModeServer)) {
            [_deflater reset];
        }
        ...
        // set rsv1 mask
        headerBytes[0] |= BJLPSWebSocketRsv1Mask;
    }
    ...
    if(_mode == BJLPSWebSocketModeClient) {
        ...
        headerBytes[1] |= BJLPSWebSocketMaskMask;
...
- (NSInteger)readBytes:(void *)bytes maxLength:(NSUInteger)maxLength error:(NSError *__autoreleasing *)outError {
    ...
    switch(_state) {
        ...
        // HANDSHAKE RESPONSE
        //
        case BJLPSWebSocketDriverStateHandshakeResponse: {
            NSAssert(maxLength > 0, @"Must have 1 or more bytes");
            NSAssert(_state == BJLPSWebSocketDriverStateHandshakeResponse, @"Invalid state for reading handshake response");
            
            void* boundary = memmem(bytes, maxLength, "\r\n\r\n", 4);
            if (boundary == NULL) {
                // do not allow too much data for headers
                if(maxLength >= 16384) {
                    BJLPSWebSocketSetOutError(outError, BJLPSWebSocketErrorCodeHandshakeFailed, @"HTTP headers did not finish after reading 16384 bytes");
                    return -1;
                }
                return 0;
            }
            ...
            if (!CFHTTPMessageAppendBytes(msg, (const UInt8 *)bytes, preBoundaryLength)) {
                BJLPSWebSocketSetOutError(outError, BJLPSWebSocketErrorCodeHandshakeFailed, @"Not a valid HTTP response");
                CFRelease(msg);
                return -1;
            }
            ...
            if(!CFHTTPMessageIsHeaderComplete(msg)) {
                BJLPSWebSocketSetOutError(outError, BJLPSWebSocketErrorCodeHandshakeFailed, @"HTTP headers found CRLFCRLF but not complete");
                CFRelease(msg);
                return -1;
            }
            ...
            if(statusCode != 101) {
                if(outError) {
                    ...
                    NSDictionary* userInfo = @{NSLocalizedDescriptionKey: desc,
                                               ...
                                               BJLPSHTTPStatusErrorKey: @(statusCode),
                                               BJLPSHTTPResponseErrorKey: (__bridge id)msg};
                    *outError = [NSError errorWithDomain:BJLPSWebSocketErrorDomain
                                                    code:BJLPSWebSocketErrorCodeHandshakeFailed
                                                userInfo:userInfo];
                }
                return - 1;
            }
            ...
            if(![headers[@"Sec-WebSocket-Accept"] isEqualToString:[self acceptHeaderForKey:_handshakeSecKey]]) {
                BJLPSWebSocketSetOutError(outError, BJLPSWebSocketErrorCodeHandshakeFailed, @"Invalid Sec-WebSocket-Accept");
                return -1;
            }
            
            // validate version
            if(headers[@"Sec-WebSocket-Version"] && ![headers[@"Sec-WebSocket-Version"] isEqualToString:@"13"]) {
                BJLPSWebSocketSetOutError(outError, BJLPSWebSocketErrorCodeHandshakeFailed, @"Invalid Sec-WebSocket-Version");
                return -1;
            }
            ...
            if (protocolRequest) {
                ...
                if(!_protocol || ![protocolComponents containsObject:_protocol]) {
                    BJLPSWebSocketSetOutError(outError, BJLPSWebSocketErrorCodeHandshakeFailed,
            ...
            // per-message deflate
            if(![self pmdConfigureWithExtensionsHeaderComponents:extensionComponents]) {
                BJLPSWebSocketSetOutError(outError, BJLPSWebSocketErrorCodeHandshakeFailed, @"permessage-deflate could not negotiate parameters");
                return -1;
            }
        ...
        // FRAME HEADER
        //
        case BJLPSWebSocketDriverStateFrameHeader: {
            NSAssert(maxLength > 0, @"Must have 1 or more bytes");
            ...
            BOOL fin = !!(header[0] & BJLPSWebSocketFinMask);
            BOOL rsv1 = !!(header[0] & BJLPSWebSocketRsv1Mask);
            BOOL rsv2 = !!(header[0] & BJLPSWebSocketRsv2Mask);
            BOOL rsv3 = !!(header[0] & BJLPSWebSocketRsv3Mask);
            BJLPSWebSocketOpCode opcode = (header[0] & BJLPSWebSocketOpCodeMask);
            BOOL masked = !!(header[1] & BJLPSWebSocketMaskMask);
            uint64_t payloadLength = (header[1] & BJLPSWebSocketPayloadLenMask);
            ...
            // validate opcode
            if(!BJLPSWebSocketOpCodeIsValid(opcode)) {
                BJLPSWebSocketSetOutError(outError, BJLPSWebSocketStatusCodeProtocolError, @"Invalid opcode");
                return -1;
            }
            
            // determine if control frame
            BOOL control = BJLPSWebSocketOpCodeIsControl(opcode);
            
            // validate control frame
            if(control) {
                // control frames must be final
                if(!fin) {
                    BJLPSWebSocketSetOutError(outError, BJLPSWebSocketStatusCodeProtocolError, @"Control frames must be final");
                    return -1;
                }
                // control frames must not have any reserved bits set
                if(rsv1 || rsv2 || rsv3) {
                    BJLPSWebSocketSetOutError(outError, BJLPSWebSocketStatusCodeProtocolError, @"Control frames must not use reserved bits");
                    return -1;
                }
                // control frames must not have payload >= 126
                if(payloadLength >= 126) {
                    BJLPSWebSocketSetOutError(outError, BJLPSWebSocketStatusCodeProtocolError, @"Control frames payload must < 126");
                    return -1;
                }
            }
            // validate data frame
                ...
                if(opcode == BJLPSWebSocketOpCodeContinuation && _frames.count == 0) {
                    BJLPSWebSocketSetOutError(outError, BJLPSWebSocketStatusCodeProtocolError, @"Data continuation frames must follow an initial data frame");
                    return -1;
                }
                // non data continuation frames must not follow an initial data frame
                if(opcode != BJLPSWebSocketOpCodeContinuation && _frames.count > 0) {
                    BJLPSWebSocketSetOutError(outError, BJLPSWebSocketStatusCodeProtocolError, @"Data frames must not follow an initial data frame unless continuations");
                    return -1;
                }
                // rsv1 only set if permessage-deflate is enabled
                if(rsv1 && !_pmdEnabled) {
                    BJLPSWebSocketSetOutError(outError, BJLPSWebSocketStatusCodeProtocolError, @"Data frames must only use rsv1 bit if permessage-deflate extension is on");
                    return -1;
                }
                // rsv2 and rsv3 must never be set
                if(rsv2 || rsv3) {
                    BJLPSWebSocketSetOutError(outError, BJLPSWebSocketStatusCodeProtocolError, @"Data frames must never use rsv2 or rsv3 bits");
                    return -1;
                }
            }
            
            // ensure not masked if client mode
            if(masked && _mode == BJLPSWebSocketModeClient) {
                BJLPSWebSocketSetOutError(outError, BJLPSWebSocketStatusCodeProtocolError, @"Frames must never be masked from server");
                return -1;
            }
            // ensure masked if server mode
            if(!masked && _mode == BJLPSWebSocketModeServer) {
                BJLPSWebSocketSetOutError(outError, BJLPSWebSocketStatusCodeProtocolError, @"Frames must always be masked from client");
                return -1;
            }
            
            // create frame
            BJLPSWebSocketFrame *frame = [[BJLPSWebSocketFrame alloc] init];
            frame->fin = fin;
            frame->rsv1 = rsv1;
            frame->rsv2 = rsv2;
            frame->rsv3 = rsv3;
            ...
            if(!frame->control && _frames.count > 0) {
                BJLPSWebSocketFrame *lastFrame = [_frames lastObject];
            ...
            if(headerExtraLength > 0) {
                _state = BJLPSWebSocketDriverStateFrameHeaderExtra;
            } else if(payloadLength > 0) {
                _state = BJLPSWebSocketDriverStateFramePayload;
            } else {
                _state = BJLPSWebSocketDriverStateFrameHeader;
        ...
        // FRAME HEADER EXTRA
        //
        case BJLPSWebSocketDriverStateFrameHeaderExtra: {
            NSAssert(maxLength > 0, @"Must have 1 or more bytes");
            
            // get current frame
            BJLPSWebSocketFrame *frame = [_frames lastObject];
        ...
        // FRAME PAYLOAD
        //
        case BJLPSWebSocketDriverStateFramePayload: {
            NSAssert(maxLength > 0, @"Must have 1 or more bytes");
            ...
            if(_mode == BJLPSWebSocketModeServer) {
            ...
            if(frame->pmd) {
                ...
                if((_pmdClientNoContextTakeover && _mode == BJLPSWebSocketModeServer) ||
                   (_pmdServerNoContextTakeover && _mode == BJLPSWebSocketModeClient)) {
                    [_inflater reset];
                }
            ...
            if(frame->opcode == BJLPSWebSocketOpCodeText) {
                uint8_t *bytes = (uint8_t *)(frame->buffer.bytes + offset);
                for(NSUInteger i = 0; i < frame->buffer.length - offset; ++i) {
                    // get validation result
                    BJLPSWebSocketUTF8DecoderDecode(&_utf8DecoderState, &_utf8DecoderCodePoint, *(bytes + i));
                    
                    // read bad code point
                    if(_utf8DecoderState == BJLPSWebSocketUTF8DecoderReject) {
                        BJLPSWebSocketSetOutError(outError, BJLPSWebSocketStatusCodeInvalidUTF8, @"Invalid UTF-8");
                        return -1;
                    }
                }
...
- (BOOL)processFramesAndDelegate:(NSError *__autoreleasing *)outError {
    ...
    switch(frame->opcode) {
        ...
        case BJLPSWebSocketOpCodeClose:
            if(frame->buffer.length >= 2) {
                uint16_t closeCode = 0;
                [frame->buffer getBytes:&closeCode length:sizeof(closeCode)];
                closeCode = EndianU16_BtoN(closeCode);
                if(!BJLPSWebSocketCloseCodeIsValid(closeCode)) {
                    BJLPSWebSocketSetOutError(outError, BJLPSWebSocketStatusCodeProtocolError, @"Invalid close code");
                    return NO;
                }
                NSString *reason = nil;
                if(frame->buffer.length > 2) {
                    reason = [[NSString alloc] initWithBytes:frame->buffer.mutableBytes + sizeof(uint16_t)
                                                    ...
                                                    encoding:NSUTF8StringEncoding];
                    if(!reason) {
                        BJLPSWebSocketSetOutError(outError, BJLPSWebSocketStatusCodeProtocolError, @"Invalid close reason; must be UTF-8");
                        return NO;
                    }
                }
                [_delegate driver:self didCloseWithCode:closeCode reason:reason];
            } else if(frame->buffer.length >= 1) {
                BJLPSWebSocketSetOutError(outError, BJLPSWebSocketStatusCodeProtocolError, @"Invalid close payload");
                return NO;
            } else {
                [_delegate driver:self didCloseWithCode:BJLPSWebSocketStatusCodeNoStatusReceived reason:nil];
            }
            break;
        case BJLPSWebSocketOpCodeContinuation:
            return YES;
    }
...
+ (NSError*)errorWithCode:(NSInteger)code reason:(NSString *)reason {
    if (reason == nil) {
        ...
        if (code >= BJLPSWebSocketStatusCodeNormal && code <= BJLPSWebSocketStatusCodeMessageTooBig) {
            reason = kStatusNames[code - BJLPSWebSocketStatusCodeNormal];
        }
    }
    NSDictionary *userInfo = reason ? @{NSLocalizedDescriptionKey: reason} : nil;
    return [NSError errorWithDomain:BJLPSWebSocketErrorDomain code:code userInfo:userInfo];
}
...
- (NSArray *)pmdExtensionsHeaderComponents {
    if(_pmdEnabled) {
        ...
        // server mode
        else if(_mode == BJLPSWebSocketModeServer) {
...
- (BOOL)pmdConfigureWithExtensionsHeaderComponents:(NSOrderedSet *)components {
    ...
    for(NSString *component in components) {
        ...
        } else if([subcomponents[0] isEqualToString:@"client_no_context_takeover"] && _mode == BJLPSWebSocketModeClient) {
            _pmdClientNoContextTakeover = YES;
        } else if([subcomponents[0] isEqualToString:@"server_no_context_takeover"] && _mode == BJLPSWebSocketModeClient) {
    ...
    if (_pmdEnabled) {
        if(_mode == BJLPSWebSocketModeClient) {
            _inflater = [[BJLPSWebSocketInflater alloc] initWithWindowBits:_pmdServerWindowBits];
            _deflater = [[BJLPSWebSocketDeflater alloc] initWithWindowBits:_pmdClientWindowBits memoryLevel:8];
        } else {
            _inflater = [[BJLPSWebSocketInflater alloc] initWithWindowBits:_pmdClientWindowBits];
            _deflater = [[BJLPSWebSocketDeflater alloc] initWithWindowBits:_pmdServerWindowBits memoryLevel:8];
        }
    }
...
- (NSString *)acceptHeaderForKey:(NSString *)key {
    ...
    NSString *combined = [key stringByAppendingString:BJLPSWebSocketGUID];
