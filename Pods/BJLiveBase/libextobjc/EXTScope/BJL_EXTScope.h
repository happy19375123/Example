 ...
 */
#define bjl_onExit \
    __strong bjl_cleanupBlock_t metamacro_concat(bjl_exitBlock_, __LINE__) __attribute__((cleanup(bjl_executeCleanupBlock), unused)) = ^
 ...
 */
#define bjl_weakify(...) \
    metamacro_foreach_cxt(bjl_weakify_,, __weak, __VA_ARGS__)

/**
 * Like #weakify, but uses \c __unsafe_unretained instead, for targets or
 * classes that do not support weak references.
 */
#define bjl_unsafeify(...) \
    metamacro_foreach_cxt(bjl_weakify_,, __unsafe_unretained, __VA_ARGS__)
 ...
 * @endcode
 */
#define bjl_strongify(...) \
    _Pragma("clang diagnostic push") \
    _Pragma("clang diagnostic ignored \"-Wshadow\"") \
    metamacro_foreach(bjl_strongify_,, __VA_ARGS__) \
    _Pragma("clang diagnostic pop")

/*** implementation details follow ***/
typedef void (^bjl_cleanupBlock_t)(void);

void bjl_executeCleanupBlock (__strong bjl_cleanupBlock_t *block);

#define bjl_weakify_(INDEX, CONTEXT, VAR) \
    CONTEXT __typeof__(VAR) metamacro_concat(VAR, _weak_) = (VAR);

#define bjl_strongify_(INDEX, VAR) \
