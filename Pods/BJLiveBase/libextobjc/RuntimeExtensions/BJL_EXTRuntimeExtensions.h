 ...
 */
typedef void (*bjl_failedMethodCallback)(Class, Method);

/**
 * Used with #bjl_injectMethods to determine injection behavior.
 */
typedef NS_OPTIONS(NSUInteger, bjl_methodInjectionBehavior) {
    /**
     * Indicates that any existing methods on the destination class should be
     * overwritten.
     */
    bjl_methodInjectionReplace                  = 0x00,

    /**
     * Avoid overwriting methods on the immediate destination class.
     */
    bjl_methodInjectionFailOnExisting           = 0x01,
    
    /**
     * Avoid overriding methods implemented in any superclass of the destination
     * class.
     */
    bjl_methodInjectionFailOnSuperclassExisting = 0x02,

    /**
     * Avoid overwriting methods implemented in the immediate destination class
     * or any superclass. This is equivalent to
     * <tt>bjl_methodInjectionFailOnExisting | bjl_methodInjectionFailOnSuperclassExisting</tt>.
     */
    bjl_methodInjectionFailOnAnyExisting        = 0x03,

    /**
     * Ignore the \c +load class method. This does not affect instance method
     * injection.
     */
    bjl_methodInjectionIgnoreLoad = 1U << 2,

    /**
     * Ignore the \c +initialize class method. This does not affect instance method
     * injection.
     */
    bjl_methodInjectionIgnoreInitialize = 1U << 3
};

/**
 * A mask for the overwriting behavior flags of #bjl_methodInjectionBehavior.
 */
static const bjl_methodInjectionBehavior bjl_methodInjectionOverwriteBehaviorMask = 0x3;
...
typedef enum {
     ...
     */
    bjl_propertyMemoryManagementPolicyAssign = 0,

    /**
     * The value is retained.
     */
    bjl_propertyMemoryManagementPolicyRetain,

    /**
     * The value is copied.
     */
    bjl_propertyMemoryManagementPolicyCopy
} bjl_propertyMemoryManagementPolicy;
...
typedef struct {
     ...
     * #bjl_propertyMemoryManagementPolicyAssign if #readonly is \c YES.
     */
    bjl_propertyMemoryManagementPolicy memoryManagementPolicy;
 ...
 */
Class *bjl_copySubclassList (Class aClass, unsigned *subclassCount);
 ...
 */
#define bjl_getIvar(OBJ, IVAR, TYPE) \
 ...
 * or floating-point types.
 */
#define bjl_getIvarByName(OBJ, NAME, TYPE) \
    bjl_getIvar((OBJ), class_getInstanceVariable(object_getClass((OBJ)), (NAME)), TYPE)
 ...
 */
unsigned bjl_injectMethods (Class aClass, Method *methods, unsigned count, bjl_methodInjectionBehavior behavior, bjl_failedMethodCallback failedToAddCallback);

/**
 * Invokes #bjl_injectMethods with the instance methods and class methods from
 * \a srcClass. #bjl_methodInjectionIgnoreLoad is added to #behavior for class
 * method injection.
 *
 * Returns whether all methods were added successfully. For each method that fails
 * to be added, \a failedToAddCallback (if provided) is invoked.
 ...
 */
BOOL bjl_injectMethodsFromClass (Class srcClass, Class dstClass, bjl_methodInjectionBehavior behavior, bjl_failedMethodCallback failedToAddCallback);
 ...
 *
 * @li Each protocol is loaded with #bjl_loadSpecialProtocol and a custom block
 ...
 * #bjl_specialProtocolReadyForInjection.
 ...
 * #bjl_specialProtocolReadyForInjection as well.
 *
