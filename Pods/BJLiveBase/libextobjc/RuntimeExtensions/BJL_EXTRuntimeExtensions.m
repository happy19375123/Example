...
#import "BJL_EXTRuntimeExtensions.h"
#import <ctype.h>
...
// typedef NSMethodSignature *(*bjl_methodSignatureForSelectorIMP)(id, SEL, SEL);
typedef void (^bjl_specialProtocolInjectionBlock)(Class);

// a `const char *` equivalent to system struct objc_method_description
typedef struct {
    SEL name;
    const char *types;
} bjl_methodDescription;
...
typedef struct {
    ...
    BOOL ready;
} bjl_EXTSpecialProtocol;

// the full list of special protocols (an array of bjl_EXTSpecialProtocol structs)
static bjl_EXTSpecialProtocol * restrict specialProtocols = NULL;
...
// the number of bjl_EXTSpecialProtocols which have been marked as ready for
 ...
 */
static void bjl_injectSpecialProtocols (void) {
    /*
     ...
     */
    qsort_b(specialProtocols, specialProtocolCount, sizeof(bjl_EXTSpecialProtocol), ^(const void *a, const void *b){
        // if the pointers are equal, it must be the same protocol
        if (a == b)
            return 0;

        const bjl_EXTSpecialProtocol *protoA = a;
        const bjl_EXTSpecialProtocol *protoB = b;

        // A higher return value here means a higher priority
        int (^protocolInjectionPriority)(const bjl_EXTSpecialProtocol *) = ^(const bjl_EXTSpecialProtocol *specialProtocol){
            int runningTotal = 0;
	...
	// use this instead of bjl_copyClassList() to avoid sending +initialize to
    ...
    @autoreleasepool {
        ...
        for (size_t i = 0;i < specialProtocolCount;++i) {
            ...
            // from the structure
            bjl_specialProtocolInjectionBlock injectionBlock = (__bridge_transfer id)specialProtocols[i].injectionBlock;
...
unsigned bjl_injectMethods (
    Class aClass,
    Method *methods,
    unsigned count,
    bjl_methodInjectionBehavior behavior,
    bjl_failedMethodCallback failedToAddCallback
) {
    ...
    @autoreleasepool {
        ...
        if (!isMeta) {
            ...
            behavior &= ~(bjl_methodInjectionIgnoreLoad | bjl_methodInjectionIgnoreInitialize);
        }
        ...
        for (unsigned methodIndex = 0;methodIndex < count;++methodIndex) {
            ...
            if (behavior & bjl_methodInjectionIgnoreLoad) {
            ...
            if (behavior & bjl_methodInjectionIgnoreInitialize) {
            ...
            switch (behavior & bjl_methodInjectionOverwriteBehaviorMask) {
            case bjl_methodInjectionFailOnExisting:
                success = class_addMethod(aClass, methodName, impl, type);
                break;

            case bjl_methodInjectionFailOnAnyExisting:
            ...
            case bjl_methodInjectionReplace:
                class_replaceMethod(aClass, methodName, impl, type);
                break;

            case bjl_methodInjectionFailOnSuperclassExisting:
                {
...
BOOL bjl_injectMethodsFromClass (
    Class srcClass,
    Class dstClass,
    bjl_methodInjectionBehavior behavior,
    bjl_failedMethodCallback failedToAddCallback)
{
    ...
    // ignore +load
    behavior |= bjl_methodInjectionIgnoreLoad;
    addedCount = bjl_injectMethods(
...
Class bjl_classBeforeSuperclass (Class receiver, Class superclass) {
...
Class *bjl_copyClassList (unsigned *count) {
...
unsigned bjl_addMethods (Class aClass, Method *methods, unsigned count, BOOL checkSuperclasses, bjl_failedMethodCallback failedToAddCallback) {
    bjl_methodInjectionBehavior behavior = bjl_methodInjectionFailOnExisting;
    if (checkSuperclasses)
        behavior |= bjl_methodInjectionFailOnSuperclassExisting;

    return bjl_injectMethods(
        aClass,
        methods,
        count,
        behavior,
...
BOOL bjl_addMethodsFromClass (Class srcClass, Class dstClass, BOOL checkSuperclasses, bjl_failedMethodCallback failedToAddCallback) {
    bjl_methodInjectionBehavior behavior = bjl_methodInjectionFailOnExisting;
    if (checkSuperclasses)
        behavior |= bjl_methodInjectionFailOnSuperclassExisting;
    
    return bjl_injectMethodsFromClass(srcClass, dstClass, behavior, failedToAddCallback);
}

BOOL bjl_classIsKindOfClass (Class receiver, Class aClass) {
    while (receiver) {
        if (receiver == aClass)
            return YES;
...
Class *bjl_copyClassListConformingToProtocol (Protocol *protocol, unsigned *count) {
    Class *allClasses;
    ...
    @autoreleasepool {
        ...
        allClasses = bjl_copyClassList(&classCount);
...
bjl_propertyAttributes *bjl_copyPropertyAttributes (objc_property_t property) {
    ...
    bjl_propertyAttributes *attributes = calloc(1, sizeof(bjl_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate bjl_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        case 'C':
            attributes->memoryManagementPolicy = bjl_propertyMemoryManagementPolicyCopy;
            break;

        case '&':
            attributes->memoryManagementPolicy = bjl_propertyMemoryManagementPolicyRetain;
            break;
...
Class *bjl_copySubclassList (Class targetClass, unsigned *subclassCount) {
    unsigned classCount = 0;
    Class *allClasses = bjl_copyClassList(&classCount);
...
Method bjl_getImmediateInstanceMethod (Class aClass, SEL aSelector) {
...
BOOL bjl_getPropertyAccessorsForClass (objc_property_t property, Class aClass, Method *getter, Method *setter) {
    bjl_propertyAttributes *attributes = bjl_copyPropertyAttributes(property);
    if (!attributes)
        return NO;
...
NSMethodSignature *bjl_globalMethodSignatureForSelector (SEL aSelector) {
    ...
    static bjl_methodDescription volatile methodDescriptionCache[selectorCacheLength];
    ...
    bjl_methodDescription methodDesc;
    ...
    methodDesc = (bjl_methodDescription){.name = NULL, .types = NULL};

    uint classCount = 0;
    Class *classes = bjl_copyClassList(&classCount);
    ...
    if (classes) {
        @autoreleasepool {
            ...
            for (uint i = 0;i < classCount;++i) {
                ...
                if (method) {
                    methodDesc = (bjl_methodDescription){.name = aSelector, .types = method_getTypeEncoding(method)};
                    break;
                }
            }
        }
    ...
    if (!methodDesc.name) {
        ...
        if (protocols) {
            ...
            for (uint i = 0;i < protocolCount;++i) {
                ...
                if (objcMethodDesc.name) {
                    methodDesc = (bjl_methodDescription){.name = objcMethodDesc.name, .types = objcMethodDesc.types};
                    break;
                }
            }
            free(protocols);
...
BOOL bjl_loadSpecialProtocol (Protocol *protocol, void (^injectionBehavior)(Class destinationClass)) {
    @autoreleasepool {
        ...
        // bjl_injectSpecialProtocols()
        #ifndef __clang_analyzer__
        bjl_specialProtocolInjectionBlock copiedBlock = [injectionBehavior copy];

        // construct a new bjl_EXTSpecialProtocol structure and add it to the first
        // empty space in the array
        specialProtocols[specialProtocolCount] = (bjl_EXTSpecialProtocol){
            .protocol = protocol,
...
void bjl_specialProtocolReadyForInjection (Protocol *protocol) {
    @autoreleasepool {
        NSCParameterAssert(protocol != nil);
...
void bjl_removeMethod (Class aClass, SEL methodName) {
    Method existingMethod = bjl_getImmediateInstanceMethod(aClass, methodName);
    if (!existingMethod) {
        return;
    }
...
void bjl_replaceMethods (Class aClass, Method *methods, unsigned count) {
    bjl_injectMethods(
        aClass,
        methods,
        count,
        bjl_methodInjectionReplace,
        NULL
    );
}

void bjl_replaceMethodsFromClass (Class srcClass, Class dstClass) {
    bjl_injectMethodsFromClass(srcClass, dstClass, bjl_methodInjectionReplace, NULL);
}

NSString *bjl_stringFromTypedBytes (const void *bytes, const char *encoding) {
    switch (*encoding) {
