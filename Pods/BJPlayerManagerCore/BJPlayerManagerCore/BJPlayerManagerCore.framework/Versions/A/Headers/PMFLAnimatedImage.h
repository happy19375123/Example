...
#import "PMFLAnimatedImageView.h"
...
extern const NSTimeInterval kPMFLAnimatedImageDelayTimeIntervalMinimum;

//
//  An `PMFLAnimatedImage`'s job is to deliver frames in a highly performant way and works in conjunction with `PMFLAnimatedImageView`.
...
//
@interface PMFLAnimatedImage : NSObject
...
// Pass either a `UIImage` or an `PMFLAnimatedImage` and get back its size
+ (CGSize)sizeForImage:(id)image;

// On success, the initializers return an `PMFLAnimatedImage` with all fields initialized, on failure they return `nil` and an error will be logged.
...
@interface PMFLAnimatedImage (Logging)
...
#define FLLog(logLevel, format, ...) [PMFLAnimatedImage logStringFromBlock:^NSString *{ return [NSString stringWithFormat:(format), ## __VA_ARGS__]; } withLevel:(logLevel)]
